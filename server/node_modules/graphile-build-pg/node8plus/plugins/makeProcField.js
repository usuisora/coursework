...
function makeProcField(fieldName, proc, build, {
  fieldWithHooks,
  computed = false,
  isMutation = false,
  forceList = false
  ...
  const argNames = proc.argTypeIds.reduce((prev, _, idx) => {
    if (idx >= sliceAmount && ( // Was a .slice() call
  ...
  const argModesWithOutput = ["o", // OUT,
  "b", // INOUT
  "t"];
  ...
  const argGqlTypes = argTypes.map((type, idx) => {
    ...
    if (!Type) {
      const hint = type.class ? `; this might be because no INSERT column privileges are granted on ${describePgEntity(type.class)}. You can use smart comments to tell PostGraphile to instead use the "${_chalk2.default.bold.green("base")}" input type which includes all columns:\n\n  ${sqlCommentByAddingTags(proc, {
  ...
  return fieldWithHooks(fieldName, ({
    ...
    function makeQuery(parsedResolveInfoFragment, ReturnType, sqlMutationQuery, functionAlias, parentQueryBuilder) {
      ...
      const query = queryFromResolveData(sqlMutationQuery, functionAlias, resolveData, {
        ...
        if (!isTableLike) {
          if (returnTypeTable) {
            ...
            innerQueryBuilder.select(pgTweakFragmentForTypeAndModifier(sql.fragment`${functionAlias}`, returnType, null, // We can't determine a type modifier for functions
            resolveData), "value");
          }
        }
      });
        ...
        return {
          pgQuery: queryBuilder => {
            queryBuilder.select(() => {
              ...
              const query = makeQuery(parsedResolveInfoFragment, ReturnType, sqlMutationQuery, functionAlias, queryBuilder);
    ...
    return {
      ...
      resolve: computed ? (data, _args, _context, resolveInfo) => {
        ...
        let queryResultRows;
        if (isMutation) {
          const query = makeQuery(parsedResolveInfoFragment, resolveInfo.returnType, functionAlias, functionAlias, null);
          ...
          try {
            ...
            queryResultRows = await viaTemporaryTable(pgClient, isVoid ? null : sql.identifier(returnType.namespaceName, returnType.name), sql.query`select ${isPgClass ? sql.query`${intermediateIdentifier}.*` : isPgRecord ? sql.query`${intermediateIdentifier}.*` : sql.query`${intermediateIdentifier} as ${functionAlias}`} from ${sqlMutationQuery} ${intermediateIdentifier}`, functionAlias, query, isPgClass, isPgRecord ? {
              outputArgTypes,
              outputArgNames
            } : null);
          ...
          const query = makeQuery(parsedResolveInfoFragment, resolveInfo.returnType, sqlMutationQuery, functionAlias, null);
