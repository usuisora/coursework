...
exports.default = async function viaTemporaryTable(pgClient, sqlTypeIdentifier, sqlMutationQuery, sqlResultSourceAlias, sqlResultQuery, isPgClassLike = true, pgRecordInfo = undefined) {
  ...
  if (!sqlTypeIdentifier) {
    ...
    const { rows } = await performQuery(pgClient, sql.query`
    ...
    const result = await performQuery(pgClient, sql.query`
      with ${sqlResultSourceAlias} as (
        ${sqlMutationQuery}
      )
    ...
    const convertFieldBack = isPgClassLike ? sql.query`\
          ...
          select ${sql.join(outputArgNames.map((outputArgName, idx) => sql.query`\
                    (${sqlValuesAlias}.output_value_list)[${sql.literal(idx + 1)}]::${sql.identifier(outputArgTypes[idx].namespaceName, outputArgTypes[idx].name)} as ${sql.identifier(
    // According to https://www.postgresql.org/docs/10/static/sql-createfunction.html,
    // "If you omit the name for an output argument, the system will choose a default column name."
    // In PG 9.x and 10, the column names appear to be assigned with a `column` prefix.
    ...
    const { rows: filteredValuesResults } = values.length > 0 ? await performQuery(pgClient, sql.query`\
              with ${sqlResultSourceAlias} as (
                ${convertFieldBack}
              )
              ${sqlResultQuery}
