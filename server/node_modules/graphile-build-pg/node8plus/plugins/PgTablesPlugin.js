          ...
          fields: ({ addDataGeneratorForField, Self }) => {
            ...
            if (shouldHaveNodeId) {
              ...
              addDataGeneratorForField(nodeIdFieldName, () => {
                return {
                  pgQuery: queryBuilder => {
                    queryBuilder.select(sql.fragment`json_build_array(${sql.join(primaryKeys.map(key => sql.fragment`${queryBuilder.getTableAlias()}.${sql.identifier(key.name)}`), ", ")})`, "__identifiers");
                  }
                };
              });
              fields[nodeIdFieldName] = {
                ...
                resolve(data) {
                  return data.__identifiers && getNodeIdForTypeAndIdentifiers(Self, ...data.__identifiers);
                }
              };
            }
            return fields;
